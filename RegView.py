...
class RegistryFileView(wx.Panel):
    def __init__(self, parent, registry, filename):
        ...
        test_button = wx.Button(panel_2, label="Test", pos=(10, 20))
        test_button.Bind(wx.EVT_BUTTON, self.OnTestButtonClick)
        # 설치된 응용프로그램 버튼 추가 
        self.text3 = wx.StaticText(panel_2, label = "응용프로그램 정보", pos=(10,50))
        installed_apps_button = wx.Button(panel_2, label="설치된 응용프로그램", pos=(10,70))
        installed_apps_button.Bind(wx.EVT_BUTTON, self.OnInstalledAppsButtonClick)
        # amcache.hve 버튼 추가 
        self.text3 = wx.StaticText(panel_2, label = "Amcache 분석 정보", pos=(10,110))
        amcache_analyzer_button = wx.Button(panel_2, label="Amcache.hve", pos=(10,130))
...
class InstalledApplicationsFrame(wx.Frame):
    def __init__(self, parent, results):
        ...
        self.SetSizer(sizer)
        # self.Fit()  #이 코드 넣으면 그리드에 맞춰서 창 크기가 조절되는데 너무 커져서 일단 뺐습니다
        self.Centre()
